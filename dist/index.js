...
var NODE_PATTERNS = [
  ...
  { regex: /^([\w\p{L}-]+)\(\(\((.+?)\)\)\)/u, shape: "doublecircle" },
  // A(((text)))
  // Double delimiters with mixed brackets
  { regex: /^([\w\p{L}-]+)\(\[(.+?)\]\)/u, shape: "stadium" },
  // A([text])
  { regex: /^([\w\p{L}-]+)\(\((.+?)\)\)/u, shape: "circle" },
  // A((text))
  { regex: /^([\w\p{L}-]+)\[\[(.+?)\]\]/u, shape: "subroutine" },
  // A[[text]]
  { regex: /^([\w\p{L}-]+)\[\((.+?)\)\]/u, shape: "cylinder" },
  // A[(text)]
  // Trapezoid variants — must come before plain [text]
  { regex: /^([\w\p{L}-]+)\[\/(.+?)\\\]/u, shape: "trapezoid" },
  // A[/text\]
  { regex: /^([\w\p{L}-]+)\[\\(.+?)\/\]/u, shape: "trapezoid-alt" },
  // A[\text/]
  // Asymmetric flag shape
  { regex: /^([\w\p{L}-]+)>(.+?)\]/u, shape: "asymmetric" },
  // A>text]
  // Double curly braces (hexagon) — must come before single {text}
  { regex: /^([\w\p{L}-]+)\{\{(.+?)\}\}/u, shape: "hexagon" },
  // A{{text}}
  // Single-char delimiters (last — most common, least specific)
  { regex: /^([\w\p{L}-]+)\[(.+?)\]/u, shape: "rectangle" },
  // A[text]
  { regex: /^([\w\p{L}-]+)\((.+?)\)/u, shape: "rounded" },
  // A(text)
  { regex: /^([\w\p{L}-]+)\{(.+?)\}/u, shape: "diamond" }
  // A{text}
];
var BARE_NODE_REGEX = /^([\w\p{L}-]+)/u;
...
function drawText(canvas, start, text, forceOverwrite = false) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (forceOverwrite || current === " " || current === void 0 || current === CJK_PAD) {
    ...
    if (isWideChar(ch)) {
      if (cx < canvas.length) {
        ...
        if (forceOverwrite || nextCell === " " || nextCell === void 0 || nextCell === CJK_PAD) {
...
function drawBoxWithGridDimensions(node, graph) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const vw = visualWidth(line);
    const textX = from.x + Math.floor(w / 2) - Math.ceil(vw / 2) + 1;
    let col = textX;
    for (let chIdx = 0; chIdx < line.length; chIdx++) {
      if (col >= 0 && col < box.length && startY + i >= 0 && startY + i < box[0].length) {
        box[col][startY + i] = line[chIdx];
      }
      col++;
      if (isWideChar(line[chIdx]) && col < box.length) {
        if (box[col]?.[startY + i] === " ") {
          box[col][startY + i] = CJK_PAD;
        }
        col++;
      }
    }
...
function drawMultiBox(sections, useAscii, padding = 1) {
  ...
  for (let s = 0; s < sections.length; s++) {
    ...
    for (const line of lines) {
      let col = 1 + padding;
      for (let chIdx = 0; chIdx < line.length; chIdx++) {
        canvas[col][row] = line[chIdx];
        col++;
        if (isWideChar(line[chIdx]) && col < boxWidth - 1) {
          if (canvas[col]?.[row] === " ") {
            canvas[col][row] = CJK_PAD;
          }
          col++;
        }
      }
...
function drawTextOnLine(canvas, line, label, isUpwardEdge) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const startX = middleX - Math.floor(visualWidth(lineText) / 2);
...
function drawSubgraphLabel(sg, graph) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const labelY = 1 + i;
    let col = Math.floor(width / 2) - Math.floor(visualWidth(line) / 2);
    if (col < 1) col = 1;
    for (let chIdx = 0; chIdx < line.length; chIdx++) {
      if (col < width && labelY < height) {
        canvas[col][labelY] = line[chIdx];
      }
      col++;
      if (isWideChar(line[chIdx]) && col < width && labelY < height) {
        if (canvas[col]?.[labelY] === " ") {
          canvas[col][labelY] = CJK_PAD;
        }
        col++;
      }
    }
...
function renderSequenceAscii(text, config, colorMode, theme) {
  ...
  function setText(col, row, text2, role) {
    let cx = col;
...
function renderClassAscii(text, config, colorMode, theme) {
  ...
  for (const rel of diagram.relationships) {
    ...
    if (rel.label) {
      ...
      const maxLabelWidth = Math.max(...lines2.map((l) => visualWidth(l))) + 2;
      let baseMidY;
      let idealMidX;
      if (fromBY < toTY) {
...
function renderErAscii(text, config, colorMode, theme) {
  ...
  function setText(col, row, text2, role, maxCol) {
    let cx = col;
    for (let i = 0; i < text2.length; i++) {
      const ch = text2[i];
      if (maxCol === void 0 || cx <= maxCol) {
        setC(cx, row, ch, role);
      }
      cx++;
      if (isWideChar(ch)) {
...
function elkToPositioned(elkResult, graph, mergeEdges = false) {
  ...
  const width = contentMinX < Infinity ? Math.max(contentMaxX + contentMinX, 100) : elkResult.width ?? 800;
  const height = contentMinY < Infinity ? Math.max(contentMaxY + contentMinY, 100) : elkResult.height ?? 600;
  return {
    width,
    height,
    nodes,
